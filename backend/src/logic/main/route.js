...
app.post(
  ...
  ({ body: { _id, title, tags, value, lastDate, userId, wis } }, res) =>
    Promise.all([
      ...
      value === true
        ? savePin({
            logId: mongoose.Types.ObjectId(_id),
            title,
            tags,
            created_at: getNow(),
            lastDate,
...
app.post("/saveLogs", ({ body: { pins, date, userId, wis } }, res) =>
  Promise.all(
    ...
    updatePins({ wis, userId }, { $set: { lastDate: date } })
  )
    .then(success => res.send(R.flatten(success)))
    .catch(logger)
);
